...
/**
 * Odaienv Component Route Helper
 *
 * @static
 * @package		Odaienv
 * @subpackage	Helpers
 ...
 */
abstract class OdaienvHelperRoute
{
	protected static $lookup;
	/**
	 * @param	int	The route of the odaienv
	 */
	public static function getOdaienvRoute($id, $catid)
	{
		$needles = array(
			'vms'  => array((int) $id)
		);
		//Create the link
		$link = 'index.php?option=com_odaienv&view=vms&id='. $id;
		if ($catid > 1) {
			$categories = JCategories::getInstance('Vms');
		...
		if ($item = OdaienvHelperRoute::_findItem($needles)) {
			$link .= '&Itemid='.$item;
		};
	...
	public static function getCategoryRoute($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_odaienv&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null, $identifier = 'id')
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component	= JComponentHelper::getComponent('com_odaienv');
